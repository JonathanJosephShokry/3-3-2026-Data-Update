...
import { WasabiData, Character, Member, CollectionEntry } from '../types';
import { ArrowLeft, X, Trophy, BookOpen, Package } from 'lucide-react';
import { calcHiddenScore, getMemberTotalScore, formatRarity, formatDegree } from '../utils';
...
export const WasabiCards: React.FC<WasabiCardsProps> = ({ data }) => {
  ...
  const getPackName = (packageId: string) => {
    return packageId.split('_').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' ');
  };

  const getPackScore = (packageId: string) => {
    ...
    return scores[packageId] || 5;
  };

  const packs = Array.from(new Set(data.characters.map(c => c.packageId))).map((id) => ({
    id: id as string,
    name: getPackName(id as string),
    score: getPackScore(id as string),
    characterIds: data.characters.filter(c => c.packageId === id).map(c => c.id)
  }));

  const renderPacks = () => (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
      {packs.map((pack, i) => {
        const chars = pack.characterIds.map(id => data.characters.find(c => c.id === id)).filter(Boolean) as Character[];
        const MAX_VISIBLE = 4;
            ...
            <div className="p-6 space-y-2.5">
              {visible.map(char => (
                <div key={char.id} className="flex items-center gap-3.5 p-3 bg-white/5 border-2 border-transparent rounded-xl transition-all hover:translate-x-1 hover:bg-white/10">
                  <img src={`/icons/${char.image}`} alt={char.name} className="w-12 h-12 rounded-lg object-cover" />
                ...
                >
                  <div className="flex items-center flex-shrink-0">
                    {hidden.slice(0, 3).map((char, j) => (
                      <img key={char.id} src={`/icons/${char.image}`} alt={char.name} className={`w-8 h-8 rounded-lg object-cover border-2 border-[#0d1117] transition-transform hover:scale-110 ${j > 0 ? '-ml-2.5' : ''}`} />
                    ))}
                  </div>
              ...
              {tier.map(id => {
                ...
                return (
                  <div key={id} className="flex items-center gap-3.5 p-3 bg-white/5 border-2 border-white/10 rounded-xl transition-all hover:translate-x-1 hover:bg-white/10">
                    <img src={`/icons/${char.image}`} alt={char.name} className="w-14 h-14 rounded-lg object-cover" />
                    <div className="flex-1 min-w-0">
                      <div className="text-sm font-bold text-white truncate">{char.name}</div>
                      <div className="flex gap-1 mt-1">
                        <span className={`px-2 py-0.5 rounded-lg text-[8px] font-bold uppercase tracking-wider border ${
          ...
          >
            ...
            >
              <div className="flex items-center justify-between p-6 border-b border-white/10 bg-gradient-to-br from-[#9FD356]/10 to-transparent">
                <h3 className="text-xl font-black text-white">{getPackName(selectedPackId)} – All Cards</h3>
                <button 
                  onClick={() => setSelectedPackId(null)}
