          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <section className="bg-white p-8 rounded-2xl border-2 border-[#E0E0E0] shadow-md">
              <div className="flex items-center justify-between mb-6">
                <h3 className="text-xl font-bold text-[#6B5435] flex items-center gap-2">
                  <Users size={20} /> Team Members
                </h3>
                <div className="bg-[#FAFAFA] border-2 border-[#E0E0E0] px-4 py-1.5 rounded-full text-sm font-black text-[#6B5435]">
              ...
              <div className="flex flex-wrap gap-4 justify-center">
                {project.members.map(name => {
                  ...
                  return (
                    <Link key={m.id} to={`/member/${m.id}`} className="relative group">
                      <img 
                        src={`/icons/${m.icon}`} 
                        alt={name} 
                        className={`w-16 h-16 rounded-full object-cover border-4 transition-all group-hover:scale-110 ${
                          isPM ? 'border-[#E8631A] ring-4 ring-[#E8631A]/20' : 'border-[#9FD356]'
                        }`}
                        title={name + (isPM ? ' (Project Manager)' : '')}
                        onError={(e) => {
                          const target = e.target as HTMLImageElement;
                          target.src = `data:image/svg+xml,%3Csvg xmlns=%22http://www.w3.org/2000/svg%22 width=%22100%22 height=%22100%22%3E%3Crect fill=%22%239FD356%22 width=%22100%22 height=%22100%22/%3E%3Ctext x=%2250%25%22 y=%2250%25%22 font-size=%2240%22 text-anchor=%22middle%22 dy=%22.3em%22 fill=%22white%22%3E${name.charAt(0)}%3C/text%3E%3C/svg%3E`;
                        }}
                      />
                      {isPM && (
                        <div className="absolute -top-1 -right-1 bg-[#E8631A] text-white text-[10px] font-black px-2 py-0.5 rounded-full border-2 border-white shadow-sm">PM</div>
                      )}
                      <div className="absolute -bottom-6 left-1/2 -translate-x-1/2 text-[10px] font-bold text-[#6B5435] opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap">{name}</div>
                    </Link>
                  );
                })}
              </div>
            ...
            {/* Waiting List Section (Update 11) */}
